...
csv_file = open(f"posture_report_{today}.csv",
                "w", newline="", encoding="utf-8")
writer = csv.writer(csv_file)
writer.writerow(["Time", "curr_Y", "neck_angle",
...
def show_popup_async(message):
    ...
    def _popup():
        ...
        ctypes.windll.user32.MessageBoxW(
            0, message, "AI Posture Warning", 0x30)
...
def draw_ui(frame, state):
    ...
    # 背景框
    cv2.rectangle(frame, (pad, pad), (pad + box_w,
                  pad + box_h), (40, 40, 40), -1)
    cv2.rectangle(frame, (pad, pad), (pad + box_w,
                  pad + box_h), (110, 110, 110),  1)
    ...
    if state["bad_elapsed"] > 0:
        ...
        bar_y = y + 6
        cv2.rectangle(frame, (x, bar_y), (x + bar_full,
                      bar_y + 10), (80, 80, 80), -1)
        cv2.rectangle(frame, (x, bar_y), (x + bar_fill,
                      bar_y + 10), (0, 165, 255), -1)
        cv2.putText(frame, f"Bad: {state['bad_elapsed']:.0f}s / {BAD_POSTURE_WARN_SEC}s",
                    (x, bar_y + 24), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 165, 255), 1)
    y += 42

    # 久坐 & 好姿勢比例
    sit_color = (0, 0, 255) if state["sitting_min"] >= SITTING_LIMIT_MIN else (
        200, 200, 200)
...
while cap.isOpened():
    ...
    if result.pose_landmarks:
        ...
        #     has_warned = False
        #     print(
        #         f"Calibrated → Y={base['Y']:.4f}  neck={base['neck_angle']:.1f}°  shoulder={base['shoulder_diff']:.4f}")
        # --- 校準 ---
        if key in (ord("c"), ord("C")):
            nose = [lm[0].x, lm[0].y]
            l_ear = [lm[7].x, lm[7].y]
            ...
            # 1. 計算基準值
            base_Y_val = (lm[11].y + lm[12].y) / 2 - (lm[9].y + lm[10].y) / 2
            base_angle_val = calc_angle(nose, l_ear, l_shoulder)
            base_sh_diff = abs(lm[11].y - lm[12].y)

            base = {
                "Y": base_Y_val,
                "neck_angle": base_angle_val,
                "shoulder_diff": base_sh_diff,
            }

            # 2. 【新增】立刻將這筆基準資料寫入 CSV
            t = time.strftime("%H:%M:%S")
            # 狀態設為 "CALIBRATED (BASE)"，Issues 留空
            writer.writerow([t, f"{base_Y_val:.4f}", f"{base_angle_val:.1f}",
                             f"{base_sh_diff:.4f}", "GOOD POSTURE", "CALIBRATED_BASE"])
            csv_file.flush()  # 確保立刻寫入硬碟

            is_calibrated = True
            session_start = time.time()
            last_save_time = time.time()  # 重設存檔計時，避免下一秒重複存
            bad_posture_start = None
            has_warned = False

            print(
                f"Calibrated & Saved Base → Y={base['Y']:.4f} neck={base['neck_angle']:.1f}°")
        # --- 分析 ---
        if is_calibrated:
            ...
            if status == "GOOD POSTURE":
                ...
                if bad_elapsed >= BAD_POSTURE_WARN_SEC and not has_warned:
                    has_warned = True
                    show_popup_async(
                        f"Bad posture for {BAD_POSTURE_WARN_SEC}s!\nPlease sit up straight.")
            ...
            if sitting_sec >= SITTING_LIMIT_MIN * 60 and (time.time() - last_stand_reminder) > 60:
                ...
                show_popup_async(
                    f"You have been sitting for {SITTING_LIMIT_MIN} minutes.\nPlease stand up and stretch!")
